...
try {
  ...
  // Measure the browser's actual CSS mm→px conversion
  ...
  await page.evaluate(({ pw, ph }) => {
    ...
    // Force each page to exact A4 pixel dimensions
    // The flex layout keeps header at top, footer at bottom,
    // and .page-body fills the middle (overflowing content clips)
    ...
    // Ensure page-body clips overflow content
  ...
  // Keep viewport wide (1200px) so responsive breakpoints don't trigger.
  // element.screenshot() captures only the .pdf-page element (210mm wide).
